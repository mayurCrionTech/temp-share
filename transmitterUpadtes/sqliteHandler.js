...
class SQLiteHandler {
  ...
  // One file per hour, named by date+hour so listing/sorting the data
  // dir is enough to see how many hours are on disk. Computed in IST
  // regardless of the machine's local timezone, so behavior is
  // consistent whether this runs as a Windows service, a Linux box,
  // or in a container set to UTC.
  getHourKey(date = new Date()) {
    ...
    return `${parts.year}-${parts.month}-${parts.day}-${parts.hour}`; // -> "2026-08-17-14"
  }

  getDbPath(hourKey) {
    return path.join(this.baseDir, `plc_data_${hourKey}.db`);
  }
  ...
  listDbFiles() {
    ...
    return fs
      ...
      .filter((f) => /^plc_data_\d{4}-\d{2}-\d{2}-\d{2}\.db$/.test(f))
      .sort()
  ...
  // Moves any fully-synced file (other than the current live one) into
  // archiveDir. No wait - a file gets archived the first time this runs
  // after it has zero unsynced rows left, whether that's the very next
  // hour or, if the server was down for a while, however many hours it
  // took to catch up. A file with a backlog is simply skipped and
  ...
  async archiveSyncedFiles() {
    for (const dbPath of this.listDbFiles()) {
      if (dbPath === this.currentDbPath) continue; // never touch the current live file
  ...
  async getDatabase() {
    ...
    if (this.currentDb && this.currentHourKey === hourKey) {
      return this.currentDb;
    }

    // Either first run, or the hour has rolled over - close whatever
    // was open (if anything) and open/create this hour's file. Nothing
    // is deleted; the previous file is left on disk for getUnsentRecords
    ...
    const dbPath = this.getDbPath(hourKey);
    ...
    // Piggyback the archive sweep on the once-an-hour rotation moment -
  ...
  // order), not just the current hour's file. This is what makes hourly
  // rotation safe: if the sync interval falls behind and an hour boundary
  // passes while records are still unsynced, they don't get stranded in
  // the old file - they're picked up here until markAsSynced clears
  // them, however many hours back that takes.
