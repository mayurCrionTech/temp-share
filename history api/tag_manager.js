...
const ALLOWED_HISTORY_AGGREGATIONS = ["avg", "sum", "min", "max", "count", "first", "last"];

// Ladder of "nice" intervals, smallest to largest.
// Auto-selection picks the smallest one that keeps bucket count under targetPoints.
const AUTO_INTERVAL_LADDER = [
  { iso: "PT1M",  ms: 60 * 1000 },
...
function pickAutoInterval(durationMs, targetPoints = 150) {
  for (const step of AUTO_INTERVAL_LADDER) {
    if (durationMs / step.ms <= targetPoints) {
      return step;
    }
  }
  // Range is huge even at daily buckets — fall back to daily anyway.
  return AUTO_INTERVAL_LADDER[AUTO_INTERVAL_LADDER.length - 1];
}
...
async function getTagHistory(
  ...
  if (!ALLOWED_HISTORY_AGGREGATIONS.includes(aggregation)) {
    throw new Error("Invalid aggregation type");
  }
  ...
  const limitNum = Math.min(10000, Math.max(1, parseInt(limit, 10) || 100));

  const docs = await TagHistory.aggregate([
    {
      $match: {
        tag_id: tagObjectId,
          ...
          bucket: {
            $toDate: {
              $subtract: [
                ...
                {
                  $mod: [
                    { $subtract: [{ $toLong: "$timestamp" }, anchorMs] },
                    intervalMs
                  ]
                }
              ]
  ...
  const points = docs.map((d) => ({
    timestamp: d._id.bucket.toISOString(),
    value: d[aggregation]
  }));
...
const RAW_WINDOW_MS = 3.5 * 60 * 60 * 1000; // last 3.5h
const FORECAST_WINDOW_MS = 30 * 60 * 1000;  // next 30min
const DEFAULT_INTERVAL_MS = 60 * 1000;      // 1-min buckets

const ALLOWED_AGGREGATIONS = ["avg", "sum", "min", "max", "count", "first", "last"];

async function getForecastWindow({
  tagId,
  interval,
  aggregation = "avg",
  format = "compact"
  ...
  const intervalMs = interval ? parseISODuration(interval) : DEFAULT_INTERVAL_MS;

  const tagObjectId = new mongoose.Types.ObjectId(tagId);

  // ==========================================================
  // ANCHOR: everything derives from server time, not the client.
  // Not exposed in the response — kept internal only.
  // ==========================================================
  const now = new Date();
  const rawStart = new Date(now.getTime() - RAW_WINDOW_MS);
  const forecastEnd = new Date(now.getTime() + FORECAST_WINDOW_MS);

  // Both pipelines bucket relative to the SAME anchor (rawStart) so
  // bucket boundaries line up across the raw/forecast join at `now`.
  const anchorMs = rawStart.getTime();

  const buildBucketStage = () => ({
    $group: {
      _id: {
        bucket: {
          $toDate: {
  ...
  // =========================
  // RAW DATA (liveData_test) — last 3.5h
  // =========================
  ...
  const rawPromise = TagHistory.aggregate([
    {
      $match: {
        tag_id: tagObjectId,
        timestamp: { $gte: rawStart, $lte: now }
      }
    },
    { $project: { timestamp: 1, value: 1 } },
    { $sort: { timestamp: 1 } },
    buildBucketStage(),
    { $sort: { "_id.bucket": 1 } }
  ]);
  ...
  // =========================
  // FORECAST DATA (aiforecast) — next 30min
  // =========================
  ...
  const forecastPromise = forecastCollection.aggregate([
    {
      $match: {
        tagId: tagObjectId,
        timestamp: { $gte: now, $lte: forecastEnd }
      }
    },
    { $project: { timestamp: 1, value: "$predictedValue" } },
    { $sort: { timestamp: 1 } },
    buildBucketStage(),
    { $sort: { "_id.bucket": 1 } }
  ]).toArray();
  ...
  const [rawDocs, futureDocs] = await Promise.all([rawPromise, forecastPromise]);
  ...
  function getValue(doc) {
    switch (aggregation) {
      case "avg": return doc.avg;
      case "sum": return doc.sum;
      case "min": return doc.min;
      case "max": return doc.max;
      case "count": return doc.count;
      case "first": return doc.first;
      case "last": return doc.last;
      default: return null;
    }
  }
  ...
  // =========================
  // RESPONSE — same shape frontend already consumes
  // =========================
  ...
  return {
    ...
    meta,
    historical: {
      start: rawDocs.length ? rawDocs[0].firstTimestamp.toISOString() : null,
      end: rawDocs.length ? rawDocs[rawDocs.length - 1].lastTimestamp.toISOString() : null,
      values: historicalPoints.map(p => p.value)
    },
    forecast: {
      start: futureDocs.length ? futureDocs[0].firstTimestamp.toISOString() : null,
      end: futureDocs.length ? futureDocs[futureDocs.length - 1].lastTimestamp.toISOString() : null,
      values: forecastPoints.map(p => p.value)
    }
  };
}

async function getDefectPoints(pageNumber) {
...
async function updateforecastAcknowledged(reqbody, forecastDefectId, userId) {
  try {

    const { acknowledged } = reqbody;
...
async function updateAnomalyAcknowledged(reqbody, anomalyId, userId) {
  try {
...
// CR0128
async function fetchLogTags(reqData = {}) {
  const query = { source: "LOG", isActive: true };
  if (reqData.search) {
    query.tagname = { $regex: reqData.search, $options: "i" };
  }
  const tags = await TagLive.find(
    query,
    { tagname: 1, unit: 1, datatype: 1, ranges: 1, plcName: 1 }
  ).lean();
...
async function updateTagLiveFromLog(tagId, value, entryCreatedAt) {
  try {
    if (!tagId || !mongoose.Types.ObjectId.isValid(tagId)) return null;
    if (!entryCreatedAt) return null;

    const result = await TagLive.updateOne(
      {
        _id: new mongoose.Types.ObjectId(tagId),
        source: "LOG", // guard: never touch a PLC-fed tag from a log fill
        $or: [
          { "logSource.entryCreatedAt": { $exists: false } },
          { "logSource.entryCreatedAt": { $lt: new Date(entryCreatedAt) } },
        ],
      },
      {
        $set: {
          ...
          "logSource.entryCreatedAt": new Date(entryCreatedAt),
        },
      }
    );
    return result;
  } catch (error) {
    console.error("Error updating tag live from log:", error);
    return null;
  }
}
...
module.exports = {
  ...
  getTagAnomaliesFast,
  getDefectPoints,
  updateforecastAcknowledged,
  updateAnomalyAcknowledged,
  fetchLogTags, // CR0128
  updateTagLiveFromLog, // CR0128
};