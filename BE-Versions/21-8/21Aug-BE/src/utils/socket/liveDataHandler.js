...
// CR0028 Maintenance plan count increase
      ...
      sub.searchStartUTC = null;
      sub.searchEndUTC = null;
      sub.mappedTagIds = mappedTags;
      sub.currentChunkStart = now;
      sub.currentChunkEnd = new Date(now.getTime() + CHUNK_SIZE_SECONDS * 1000 - 1);
...
async function emitSecondStream(socket) {
  ...
  if (currentIndex >= totalDur) {
    ...
    const nextStart = new Date(sub.currentChunkEnd.getTime() + 1);
    const nextEnd = new Date(nextStart.getTime() + CHUNK_SIZE_SECONDS * 1000 - 1);
...
console.log(`[emitSecondStream] tag=${payload.id} val=${payload.val} idx=${currentIndex}/${totalDur}`);
