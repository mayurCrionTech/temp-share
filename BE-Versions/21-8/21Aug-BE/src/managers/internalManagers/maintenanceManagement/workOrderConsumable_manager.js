...
const workOrderConsumable = require("../../../models/mongoDB/maintenanceManagement/workOrderConsumable_model");
const mongoose = require("mongoose");

async function createWorkOrderConsumable(workOrderConsumableObject) {
  try {
    const createdWorkOrderConsumable = await mongoDbManager.insertMany(
      workOrderConsumable,
      workOrderConsumableObject
    );
    return createdWorkOrderConsumable;
  } catch (error) {
    throw error;
  }
}

async function getConsumables(reqData, workOrderId, businessUnitId) {
  try {
