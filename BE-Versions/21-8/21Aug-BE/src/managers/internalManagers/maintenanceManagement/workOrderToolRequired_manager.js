...
const workOrderToolRequired = require("../../../models/mongoDB/maintenanceManagement/workOrderToolRequired_model");
const mongoose = require("mongoose");

async function createWorkOrderToolRequired(workOrderToolRequiredObject) {
  try {
    const createdWorkOrderToolRequired = await mongoDbManager.insertMany(
      workOrderToolRequired,
      workOrderToolRequiredObject
    );
    return createdWorkOrderToolRequired;
  } catch (error) {
    throw error;
  }
}

async function getToolsRequired(reqData, workOrderId, businessUnitId) {
  try {
